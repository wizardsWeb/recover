...
# The poller loops forever against a live Supabase project. Every
# `TestClient(app)` runs the lifespan, so leaving it on would start one per
# test module. Its behaviour is covered by tests that drive it directly.
