...
echo "         az keyvault secret set --vault-name $KEY_VAULT_NAME --name gemini-api-key          --value '<key>'"
echo "         az keyvault secret set --vault-name $KEY_VAULT_NAME --name razorpay-key-id         --value '<rzp_test_...>'"
echo "         az keyvault secret set --vault-name $KEY_VAULT_NAME --name razorpay-key-secret     --value '<secret>'"
echo "         az keyvault secret set --vault-name $KEY_VAULT_NAME --name razorpay-webhook-secret --value '<secret>'"
echo "       Leave the three razorpay-* as REPLACE_ME and the agent still runs —"
echo "       every adapter simulates and says so. The Supabase two are not optional."
echo ""
echo "       Setting them needs a data-plane role. Subscription Owner is not"
echo "       enough on an RBAC vault:"
echo "         az role assignment create --assignee \$(az ad signed-in-user show --query id -o tsv) \\"
echo "           --role 'Key Vault Secrets Officer' \\"
echo "           --scope \$(az keyvault show -n $KEY_VAULT_NAME --query id -o tsv)"
